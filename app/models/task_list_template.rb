class TaskListTemplate < ActiveRecord::Base
  ...
  attr_accessor :names, :descs
  attr_accessible :name, :tasks, :names, :descs
  ...
  def to_json(options = {})
    { :id => id,
      ...
      :tasks => tasks.collect { |t| { :name => ERB::Util.html_escape(t.first), :desc => ERB::Util.html_escape(t.second) } }
    }
  end
